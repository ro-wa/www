			...
			if( $result ) {
				...
				if( $result->license === 'valid' ) {
					...
					// add extend notice if license is expiring in less than 1 month
						...
						$message .= sprintf( __( '<a href="%s">Your license is expiring in %d days, would you like to extend it?</a>', $this->product->get_text_domain() ), 'https://dannyvankooten.com/mailchimp-for-wordpress/checkout/?edd_license_key=' . esc_attr( $this->get_license_key() ) . '&download_id=899', $days_left );
					}
					...
					if( isset( $result->error ) && $result->error === 'no_activations_left' ) {
						...
						$this->set_notice( sprintf( __('You\'ve reached your activation limit. You must <a href="%s">reset</a> or <a href="%s">upgrade your license</a> to use it on this site.', $this->product->get_text_domain() ), 'http://docs.mc4wp.com/article/9-resetting-your-license', $this->product->get_tracking_url( 'license-at-limit-notice' ) ), false );
					} elseif( isset($result->error) && $result->error === "expired" ) {
						// show notice if the license is expired
						$result->license = 'expired';
						$this->set_notice( sprintf( __('Your license has expired. You must <a href="%s">renew your license</a> if you want to use it again.', $this->product->get_text_domain() ), 'https://dannyvankooten.com/mailchimp-for-wordpress/checkout/?edd_license_key=' . esc_attr( $this->get_license_key() ) . '&download_id=899', false ) );
					} else {
